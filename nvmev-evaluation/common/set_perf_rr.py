...
os.system("sudo sh -c \"echo %d %d %d > /proc/nvmev_rr/read_times\"" % (delay_initial[0], per_op_latency[0], 0))
os.system("sudo sh -c \"echo %d %d %d > /proc/nvmev_rr/write_times\"" % (delay_initial[1], per_op_latency[1], 0))
os.system("sudo sh -c \"echo %d %d > /proc/nvmev_rr/io_units\"" % (1, io_unit_shift))
